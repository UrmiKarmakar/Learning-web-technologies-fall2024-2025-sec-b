...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($result)) {
        ...
        if ($password === $user['password']) {
            ...
            $_SESSION['user_id'] = $user['id']; //v1.0.2- Changed the session variable name from id to user_id
            $_SESSION['username'] = $user['username'];
            $_SESSION['user_type'] = $user['role'];//v1.0.2- Changed the session variable name from userType to  user_type
            $_SESSION['massage']='';
